...
const ChooseMenu = ({ setOrder, selectedTable }) => {
    ...
    const handleSelectMenuItem = (item) => {
        ...
        if (existingItem) {
            setLocalOrder(order.map(o =>
                o.dishid === item.dishid ? { ...o, quantity: o.quantity + 1 } : o
            ));
        } else {
            setLocalOrder([...order, { ...item, quantity: 1 }]);
        }
    };
    ...
    const handleRemoveMenuItem = (item) => {
        ...
        if (existingItem.quantity > 1) {
            ...
            setLocalOrder(order.filter(o => o.dishid !== item.dishid));
        }
    };

    const handleConfirmOrder = () => {
        if (order.length === 0) {
            toast.warning("You Haven't Choose Any Meals!");
            return;
        }
            ...
            <ul className="menu-list">
                {menu.map((item) => (
                    <li key={item.dishid} className="menu-item">
                        ...
                        <button className="add-button" onClick={() => handleSelectMenuItem(item)}>
                ...
                <ul>
                    {order.map((item) => (
                        <li key={item.dishid}>
                            ...
                            <button className="rm-button" onClick={() => handleRemoveMenuItem(item)}>
